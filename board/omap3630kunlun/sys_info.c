...
void display_board_info(u32 btype)
{
    ...
    bootdev = __raw_readl(0x480029c0) & 0xFF;
    if(bootdev >= (sizeof(bootmode) / sizeof(bootmode[0])))
    {
        bootdev = 0;
    }
	...
	printf("OMAP%s-%s rev %d, CPU-OPP%s L3-%sMHz\n", cpu_s, sec_s, rev,
		p_cpu, p_l3);
	printf("OMAP3630KUNLUN %s Version + %s (Boot %s)\n", db_s,
		mem_s, bootmode[bootdev]);
#ifdef CONFIG_LED_INFO
