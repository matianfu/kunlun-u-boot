...
/*support I2C read from registers which address is 16 bits*/
static int i2c_read_byte2(u8 devaddr, u16 regoffset, u8 * value)
{
	int i2c_error = 0;
	u16 status;
...
/*support I2C write to registers which address is 16 bits*/
static int i2c_write_byte2(u8 devaddr,u16 regoffset, u8 value)
{
	int i2c_error = 0;
	u16 status, stat;
...
int i2c_read(uchar chip, uint addr, int alen, uchar * buffer, int len)
{
	...
	if(alen == 2){
	    if(len !=  1){
		    printf("I2C read:  read len %d not supported\n", len);
		    return 1;
		}
	    if (i2c_read_byte2(chip,addr,&buffer[0])) {
			    printf("I2C read: write subaddr %x error\n",addr);
			    i2c_init(i2c_speed, CFG_I2C_SLAVE);
			    return 1;
		}
	}
...
int i2c_write(uchar chip, uint addr, int alen, uchar * buffer, int len)
{
	...
	if(alen == 2){
	    if(len != 1){
	        printf("I2C_write:buffer len %d is not supported\r\n",len);
	        return 1;
	    }
	    if(i2c_write_byte2(chip,addr,buffer[0])){
		printf("I2C write %x to %x of chip %x: I/O error\n",buffer[0],addr,chip);
			i2c_init(i2c_speed, CFG_I2C_SLAVE);
			return 1;
		}
	}
