...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	image_header_t *hdr = &header;
       ...
       extern char *bootcase_cmdname;
       extern char *bootcase_name;
       if(cmdline && bootcase_name && bootcase_cmdname){
            if ((calib = strstr (cmdline, bootcase_cmdname)) == NULL) {
                sprintf(buf, "%s %s=%s", cmdline, bootcase_cmdname, bootcase_name);
                setenv("bootargs", buf);
            }
       }
       /*FIXME: musb can fall into deep sleep after charge full*/ 
       extern void twl4030_usb_force_standby(void);
       twl4030_usb_force_standby();
#endif

#if defined(CONFIG_ADC_CALIBRATION)
       extern char* ets_calib_env;
       extern char* wifi_mac_env;
       cmdline = getenv ("bootargs");

       if(cmdline){
            /*No calibration samples in bootargs*/
            if ((calib = strstr (cmdline, ets_calib_env)) == NULL) {
                /*get the enviroment for calibration and append into bootargs*/
                calib = getenv(ets_calib_env);
                if(calib){
                    memset(buf, 0, sizeof(buf));
                    sprintf(buf, "%s %s=%s", cmdline, ets_calib_env, calib);
                    setenv ("bootargs", buf);
                }
            }
            calib = NULL;
            /*No wifi mac in bootargs*/
            if ((calib = strstr (cmdline, wifi_mac_env)) == NULL) {
                /*get the enviroment for calibration and append into bootargs*/
                calib = getenv(wifi_mac_env);
                if(calib){
                    memset(buf, 0, sizeof(buf));
                    sprintf(buf, "%s %s=%s", cmdline, wifi_mac_env, calib);
                    setenv ("bootargs", buf);
                }
            }
	}
