...
#if !defined(CFG_ROCKBOX_FAT)
	startblock += cur_block_dev.part_offset;
#endif
...
file_fat_detectfs(void)
{
	...
	((CONFIG_COMMANDS & CFG_CMD_FAT) && defined(CONFIG_SYSTEMACE))
	printf("Interface:  ");
	switch(cur_block_dev.cur_dev->if_type) {
		...
		case IF_TYPE_SYSACE :	printf("SystemACE"); break;
...
#endif /* #if !defiined(CFG_ROCKBOX_FAT) */
