			...
			mmc_card_data *mmc_c, unsigned int *output_buf)
{
...
#if defined(CONFIG_LCD_ILI9327) || defined(BOARD_WITH_LCD_SHOW)
    int pos_x = 0,pos_y = VIDEO_FONT_HEIGHT;
#endif
	...
	while (num_sec_val) {
		err = mmc_send_cmd(mmc_cont_cur->base, MMC_CMD17,
							argument, resp);
		if (err != 1){
			//printf("mmc_send_cmd:CMD 17 Error:%d\n",err);
			return err;
		}
		...
		if (err != 1){
			printf("mmc_read_data ERROR:%d\n");
			return err;
		}
        /*indicate progress per 512 sectors*/
        if((num_sec_val % 512) == 0){
            printf(".");
#if defined(CONFIG_LCD_ILI9327) || defined(BOARD_WITH_LCD_SHOW)
            omap3_lcd_drawchars(pos_x,pos_y,(unsigned char*)".",1);
            pos_x += VIDEO_FONT_WIDTH;
            if(pos_x > LCD_WIDTH - VIDEO_FONT_WIDTH){
                pos_y += VIDEO_FONT_HEIGHT;
                if(pos_y > LCD_HEIGH - VIDEO_FONT_HEIGHT)
                    pos_y = 0;
                pos_x = 0;
            }
#endif
...
#if 0
//#if CONFIG_DRIVER_OMAP34XX_I2C /* don't compile for x-loader */
int mmc_write_block_rockbox(int dev_num, unsigned int blknr, unsigned int blkcnt,
			unsigned char *src)
{
...
int mmc_init(int slot)
{
	...
	case 0:
		...
		if(1 ==	configure_mmc(&cur_card_data[slot], &cur_controller_data[slot])){
			rc = fat_register_device(&mmc_blk_dev[slot], 1);   
        } 
		break;
	case 1:
		configure_controller(&cur_controller_data[slot], slot);
