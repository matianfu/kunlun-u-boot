...
#if defined(CONFIG_3630KUNLUN)       /* Use HTC VID&PID */
/* This is the VIA USB vendor id */
#define DEVICE_VENDOR_ID  0x0BB4
/* This is just made up.. */
#define DEVICE_PRODUCT_ID 0x0C01
/* This is just made up.. */
...
static int do_usb_req_get_descriptor(void)
{
	...
	else
	{
		...
		if (USB_DT_DEVICE == (req.wValue >> 8))
		{
			...
			d.iSerialNumber      = 0;   //for MDA tools
...
int fastboot_preboot(void)
{
	...
	for (i = 0; i < CFG_FASTBOOT_PREBOOT_LOOP_MAXIMUM; i++) {
		...
		if ((1 == CFG_FASTBOOT_PREBOOT_KEYS) &&
		    (1 == keys)) {
			if ((CFG_FASTBOOT_PREBOOT_KEY1 == key1) || (CFG_FASTBOOT_PREBOOT_KEY1 == key2))
				return 1;
		} else if ((2 == CFG_FASTBOOT_PREBOOT_KEYS) &&
			   (2 == keys)) {
			if ((CFG_FASTBOOT_PREBOOT_KEY1==key1 && CFG_FASTBOOT_PREBOOT_KEY2==key2) ||
				(CFG_FASTBOOT_PREBOOT_KEY1==key2 && CFG_FASTBOOT_PREBOOT_KEY2==key1))
				return 1;
		}
...
int fastboot_init(struct cmd_fastboot_interface *interface) 
{
    ...
    device_strings[DEVICE_STRING_MANUFACTURER_INDEX] = "VIA Telecom";
#else
...
#elif defined (CONFIG_3630KUNLUN)
	device_strings[DEVICE_STRING_PRODUCT_INDEX]       = "Kunlun";
#else
